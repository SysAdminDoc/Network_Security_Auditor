...
- [ ] **P1 — Entire scan runs synchronously on the UI thread; GUI freezes and live progress never renders.** Every check `ExecuteAsync` returns `Task.FromResult` (no `Task.Run`/`await`), and `StartScanAsync` calls `runner.RunAsync` with no `Task.Run`. Window goes "Not Responding" for the whole scan; `Progress<T>` callbacks queue behind the blocked dispatcher. Where: `ViewModels/MainViewModel.cs:213`, `Checks/CheckRunner.cs:62`, all checks. Fix: `await Task.Run(() => runner.RunAsync(...))` (or make checks genuinely async).
- [ ] **P1 — Per-check timeout cannot interrupt the blocking calls it exists for.** Timeout is only a linked `CancellationTokenSource`; sync checks observe cancellation solely at explicit `ThrowIfCancellationRequested` poll points, so a hung `ManagementObjectSearcher.Get()` / `DirectorySearcher.FindAll()` against an unreachable DC / blocking `Process` read never returns -> whole scan hangs. Where: `Checks/CheckRunner.cs:58-62`. Fix: run each check via `Task.Run` raced with `Task.Delay(timeout)` (`Task.WhenAny`), abandon the stuck task, and record only the WhenAny winner (guard against a late result from the abandoned task). Also set `DirectorySearcher.ClientTimeout`/`ServerTimeLimit` in AD checks.
...
- [ ] **P1 — Stored XSS in HTML report: `env.ComputerName` and `env.OSCaption` interpolated unescaped.** Only sink in the file missing `EscapeHtml`; OSCaption is harvested from WMI + registry `DisplayVersion` (attacker-writable on a compromised host — the tool's threat model). Where: `Export/HtmlReportGenerator.cs:63`. Fix: `EscapeHtml(...)` both.
- [ ] **P1 — Privacy mode is a false promise: only `Findings`/`Evidence` are redacted; host/domain/user/client leak verbatim everywhere else.** `computer_name`/`os`/`domain` fields, `Notes` (populated with waiver `Justification`/`ApprovedBy` before redaction), `RemediationAssignee`, `Auditor` (defaults to `Environment.UserName`), and the output filename (`SecurityAudit_{Client}_...`) all bypass the redactor. Where: `App.xaml.cs:222-243`, `MainViewModel.cs:249-275`, and every exporter (`JsonExporter:46-52`, `JsonlExporter:55-57`, `SarifExporter:84,122-124`, `OcsfExporter:96-99`, `DefectDojoExporter:96-97`, `OscalExporter:58/119/137`, `ComplianceSummaryExporter:60-62`, `IntuneExporter:46-47`, `CsvExporter:18`, `CmmcReportGenerator:43/90`). Fix: pass a redacted `EnvironmentInfo` clone (or the redactor) into exporters; redact Notes/Assignee/Auditor; pseudonymize the client in the filename.
...
- [ ] **P1 — NA03-NA07 mappings describe an older check set (DMZ / wireless-seg / VPN-seg / IDS / DNS-filter) than the catalog (wireless / net-doc / 802.1X-NAC / mgmt-isolation / guest-isolation).** Where: `Data/MitreMappings.cs:220-249`, `Data/D3FendMappings.cs:251-285`, `Data/FrameworkMappings.cs:443-503`. Fix: rewrite the five NA rows in all three files against current catalog labels.
- [ ] **P1 — BR03-BR07 mapping content is rotated off-by-one vs catalog** (restore-testing / RTO-RPO / backup-encryption / DR-plan). MITRE, D3FEND, and Framework columns all shifted. Where: `MitreMappings.cs:326-355`, `D3FendMappings.cs:374-394`, `FrameworkMappings.cs:668-732`. Fix: re-align BR03-BR07 rows.
- [ ] **P1 — PS01-PS06 carry stale "Physical Security" HIPAA/FedRAMP citations** (HIPAA 164.310 facility/media, FedRAMP PE-*) though PS is now Policies & Standards (IR plan, AUP, risk assessment, training -> should be 164.308, IR-*, AT-*). HIPAA is a first-class profile. Where: `FrameworkMappings.cs:860-931`, catalog Compliance strings `CheckCatalog.cs:982/997/1012/1027/1042/1057`. Fix: rewrite HIPAA/FedRAMP/CSF columns + catalog strings.
- [ ] **P1 — STIG column is fabricated sequential V-IDs** (V-254247->V-254338, strictly contiguous in catalog order, one per check) — synthesized placeholders emitted into compliance reports as authoritative. Where: `FrameworkMappings.cs:23-929`. Fix: source genuine STIG rule IDs or use the prose format IA11/IA12 already use; until sourced, drop from scoring surfaces. **May be Blocked** (needs real STIG data).
- [ ] **P1 — EP10 "EOL operating systems" mapped as a removable-media check** (T1091/T1052, media-protection controls). Should be T1210/T1190, SI-2/CM-8. Where: `MitreMappings.cs:150-155`, `D3FendMappings.cs:170-176`, `FrameworkMappings.cs:298-311`.
...
- [ ] **P1 — Stop Scan button can never enable; user cancellation is dead code.** `_isScanning` lacks `[NotifyCanExecuteChangedFor(nameof(StopScanCommand))]`/`(nameof(StartScanCommand))`; toolkit `RelayCommand` doesn't hook `RequerySuggested`; `NotifyCanExecuteChanged()` only runs in `finally`. Where: `MainViewModel.cs:30-32,238-247`. Fix: add the attributes to `_isScanning`.
- [ ] **P1 — Manually changing a check's Status never updates score/grade/counters.** `UpdateScoreCounts()` is never triggered by `CheckItemViewModel.Status`; no `PropertyChanged` subscription exists. The status dropdown is the manual-assessment workflow. Where: `MainViewModel.cs:582-601`, `MainWindow.xaml:226-231`. Fix: subscribe to each item's `PropertyChanged` in `LoadCheckCatalog` -> `UpdateScoreCounts`.
- [ ] **P1 — All status/severity/grade color coding is dead.** The status indicator bar is a hardcoded `#585b70` (identical gray for Pass/Fail/Partial/NA), the severity badge is always gray (CRITICAL == LOW), grade is always `TextPrimary`; VM props `StatusColor`/`SeverityColor`/`GradeColor` bind to nothing. A security tool with zero visual pass/fail signal. Where: `MainWindow.xaml:52-60,207-223,233-241`, `CheckItemViewModel.cs:43-69`. Fix: bind Background/Foreground to the color props.
...
- [ ] **P1 — Scan progress display is frozen and wrong for the whole scan.** A pre-scan `foreach` sets `IsRunning=true` on every profile check at once and overwrites `ScanStatus` to end stuck at `"Running {lastId} (1/N)"`; the progress callback never updates `ScanStatus`. No ProgressBar exists. Where: `MainViewModel.cs:204-213`. Fix: drive `IsRunning`/`ScanStatus` from the progress callback; add a determinate ProgressBar.
...
- [ ] **P2 — Final scan status permanently shows "Scan complete (0/N)"** — `completed` is read in `finally` before any queued `Progress<T>` callback has run. Where: `MainViewModel.cs:188,223-228`. Fix: compute the final count from the returned results dictionary.

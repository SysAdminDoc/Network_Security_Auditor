...
<#
.SYNOPSIS
    Network Security Auditor v4.11.2 - Professional GUI Tool
.DESCRIPTION
...
$script:AutoChecks = @{
    ...
    'EP02' = @{ Type='Local'; Label='Scan BitLocker'
        Script = {
            $sb = [System.Text.StringBuilder]::new(); $issues = 0
            if (-not (Get-Command Get-BitLockerVolume -EA SilentlyContinue)) {
                [void]$sb.AppendLine('BitLocker status unavailable: Get-BitLockerVolume is not available on this host.')
                return @{ Status='N/A'; Findings=$sb.ToString().Trim(); Evidence="Get-BitLockerVolume unavailable @ $(Get-Date -f 'yyyy-MM-dd HH:mm') on $env:COMPUTERNAME" }
            }
            try {
                $vols = @(Get-BitLockerVolume -EA Stop)
            } catch {
                [void]$sb.AppendLine("BitLocker status unavailable: $($_.Exception.Message)")
                [void]$sb.AppendLine('Run as Administrator for full disk-encryption evidence.')
                return @{ Status='Partial'; Findings=$sb.ToString().Trim(); Evidence="Get-BitLockerVolume failed @ $(Get-Date -f 'yyyy-MM-dd HH:mm') on $env:COMPUTERNAME" }
            }
            foreach ($v in $vols) {
...
# ── UK NCSC Cyber Essentials Mapping (v3.3 technical controls) ────────────────
$cyberEssentialsMap = @{
    'NP01'='Cyber Essentials v3.3: Firewalls'
    'NP02'='Cyber Essentials v3.3: Firewalls'
    'NP03'='Cyber Essentials v3.3: User access control; Firewalls'
    'NP04'='Cyber Essentials v3.3: Malware protection'
    'NP05'='Cyber Essentials v3.3: Firewalls'
    'NP06'='Cyber Essentials v3.3: Firewalls'
    'NP09'='Cyber Essentials v3.3: Firewalls'
    'NP10'='Cyber Essentials v3.3: Security update management'
    'IA01'='Cyber Essentials v3.3: User access control'
    'IA02'='Cyber Essentials v3.3: User access control'
    'IA03'='Cyber Essentials v3.3: User access control'
    'IA04'='Cyber Essentials v3.3: User access control'
    'IA05'='Cyber Essentials v3.3: User access control'
    'IA06'='Cyber Essentials v3.3: User access control'
    'IA07'='Cyber Essentials v3.3: User access control'
    'IA08'='Cyber Essentials v3.3: User access control'
    ...
    'EP04'='Cyber Essentials v3.3: Security update management'
    'EP05'='Cyber Essentials v3.3: Firewalls; Secure configuration'
    'EP06'='Cyber Essentials v3.3: Secure configuration'
    'EP07'='Cyber Essentials v3.3: Malware protection; Secure configuration'
    ...
    'CF05'='Cyber Essentials v3.3: Secure configuration'
    'CF06'='Cyber Essentials v3.3: Secure configuration'
    'CF07'='Cyber Essentials v3.3: User access control'
    'CF08'='Cyber Essentials v3.3: Malware protection'
}
foreach ($ceid in $cyberEssentialsMap.Keys) {
...
function Get-ComplianceString {
    ...
    if ($Framework -eq 'All' -or $Framework -eq 'CyberEssentials') {
        if ($script:FrameworkMap.Contains($CheckID) -and $script:FrameworkMap[$CheckID].CyberEssentials) {
            $parts += "CE: $($script:FrameworkMap[$CheckID].CyberEssentials)"
        }
    }
...
<div>Target: <strong>$([System.Net.WebUtility]::HtmlEncode((Get-RedactedIdentity $scanTarget 'HOST')))</strong></div>
                ...
                if ($sv -eq 'Fail' -or $sv -eq 'Partial') {
                    ...
                    foreach ($fw in $showFw) {
                        $val = switch ($fw) {
                            ...
                            'PCI'      { $fwData.PCI }
                            'E8'       { $fwData.E8 }
                            'CyberEssentials' { $fwData.CyberEssentials }
                            'SOC2'     { $fwData.SOC2 }
        ...
        if ($fwTarget -ne 'All') {
            ...
            foreach ($id in $fwChecks) {
                ...
                if ($sv -eq 'Fail' -or $sv -eq 'Partial') {
                    ...
                    $findings = ConvertTo-RedactedText $script:FindingsBoxes[$id].Text
                    if ($findings) {
    ...
    if ($Tier -eq 'Technical' -or $Tier -eq 'All') {
        foreach($cn in $script:AuditCategories.Keys){
            ...
            foreach($it in $cat.Items){
                ...
                $evid=ConvertTo-RedactedText $script:EvidenceBoxes[$id].Text
                ...
                if ($fwData) {
                    ...
                    if ($fwData.CyberEssentials) { $extParts += "<span style='color:#84cc16'>CE: $($fwData.CyberEssentials)</span>" }
    ...
    foreach ($cn in $script:AuditCategories.Keys) {
        ...
        foreach ($item in $cat.Items) {
            ...
            if ($fwData) {
                ...
                if ($fwData.CyberEssentials) { $compObj['Cyber_Essentials'] = $fwData.CyberEssentials }
    ...
    foreach ($cn in $script:AuditCategories.Keys) {
        ...
        foreach ($item in $cat.Items) {
            ...
            $evt = [ordered]@{
                ...
                cyber_essentials = if ($fwData) { $fwData.CyberEssentials } else { '' }
...
# OCSF Security Finding class (class_uid 2001) for vendor-neutral SIEM/MDR
        ...
        foreach ($item in $cat.Items) {
            ...
            if ($mitreData) {
                for ($ti=0; $ti -lt $mitreData.Techniques.Count; $ti++) {
                    $t = [ordered]@{ technique = [ordered]@{ uid = $mitreData.Techniques[$ti]; name = $mitreData.Techniques[$ti] }; version = $script:ExternalVersions.AttackEnterprise }
            ...
            $evt = [ordered]@{
                metadata      = [ordered]@{
                    version    = $script:ExternalVersions.OCSF
                    product    = [ordered]@{
                ...
                resources     = @(
                    [ordered]@{
                        type = 'Device'
                        name = (Get-RedactedIdentity $scanTarget 'HOST')
                        os   = [ordered]@{ name = $script:Env.OSCaption; type_id = 100; type = 'Windows' }
                    }
                )
                evidences     = if ($evidenceText) { @([ordered]@{ data = if ($evidenceText.Length -gt 2000) { (ConvertTo-RedactedText $evidenceText.Substring(0,2000)) } else { (ConvertTo-RedactedText $evidenceText) } }) } else { @() }
                attacks       = if ($attacks.Count -gt 0) { $attacks } else { $null }
                compliance    = [ordered]@{
                    requirements = @($cn)
                }
                actor         = [ordered]@{
                    user = [ordered]@{ name = (Get-RedactedIdentity $AuditorName 'AUDITOR'); type_id = 1; type = 'User' }
                }
                unmapped      = [ordered]@{
                    check_id    = $id
                    category    = $cn
...
function Export-OSCALResults {
    ...
    $oscalDoc = [ordered]@{
        'assessment-results' = [ordered]@{
            uuid = $docUuid
            metadata = [ordered]@{
                title = "Network Security Audit - $(Get-RedactedIdentity $ClientName 'CLIENT')"
                'last-modified' = $scanTs
                version = $script:ProductVersion
                'oscal-version' = $script:ExternalVersions.OSCAL
                roles = @(
                ...
                parties = @(
                    [ordered]@{
                        uuid = [guid]::NewGuid().ToString()
                        type = 'organization'
                        name = if ($AuditorName) { Get-RedactedIdentity $AuditorName 'AUDITOR' } else { 'Assessor' }
                    }
                )
                props = @(
            ...
            results = @(
                [ordered]@{
                    uuid = $resultUuid
                    title = "Assessment of $(Get-RedactedIdentity $scanTarget 'HOST')"
                    ...
                    findings = @($findings)
                    props = @(
                        [ordered]@{ name = 'scan-target'; value = (Get-RedactedIdentity $scanTarget 'HOST') }
    ...
    foreach ($cn in $script:AuditCategories.Keys) {
        ...
        foreach ($item in $cat.Items) {
            ...
            $row = [PSCustomObject][ordered]@{
                ...
                Cyber_Essentials = ConvertTo-CsvSafeText $(if ($fwData) { $fwData.CyberEssentials } else { '' })
...
function Export-IntuneCompliance {
    ...
    $output = [ordered]@{
        ...
        Auditor = (Get-RedactedIdentity $auditor 'AUDITOR')
        Environment = [ordered]@{
            Hostname = (Get-RedactedIdentity $env:COMPUTERNAME 'HOST')
            OS = $script:Env.OSCaption
            Domain = $script:Env.IsDomainJoined
            Admin = $script:Env.IsAdmin
        }
...
function Export-AttackNavigator {
    ...
    $layer = [ordered]@{
        name = "NSA - $(Get-RedactedIdentity $ClientName 'CLIENT') - $(Get-RedactedIdentity $target 'HOST')"
        versions = [ordered]@{ 'attack' = $script:ExternalVersions.AttackEnterprise; navigator = $script:ExternalVersions.AttackNavigator; layer = $script:ExternalVersions.AttackNavigator }
